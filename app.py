...
app.config['SESSION_FILE_DIR'] = f'{diretorio_atual}/usuarios/flask_session'
print(diretorio_atual)
Session(app)
socketio=SocketIO(app)
def adicionar():
    # os.getcwd()
    # subprocess.call(["python",f"{diretorio_atual}/adc_app.py"])
...
def cadastro():
  ...
  if request.method=="POST":
    status=""
    banco=sqlite3.connect(diretorio_atual+'/usuarios/usuarios.db')
    cursor=banco.cursor()
